...
def feistel(right, round_key):
    ...
    xored = [e ^ k for e, k in zip(expanded, round_key)]
...
def des_block(block, keys):
    ...
    R = block[32:]
    for key in keys:
        L, R = R, [l ^ f for l, f in zip(L, feistel(R, key))]
...
def encrypt(plaintext, key_str):
    ...
    pad_len = 8 - (len(pt_bytes) % 8)
    pt_bytes += [pad_len] * pad_len
    pt_bits = string_to_bits(pt_bytes)
    
    ct_bits = []
    for i in range(0, len(pt_bits), 64):
        block = pt_bits[i:i+64]
        ct_bits.extend(des_block(block, round_keys))
        
    str_round_keys = []
    for rk in round_keys:
        rk_hex = hex(int("".join([str(b) for b in rk]), 2))[2:].zfill(12)
        str_round_keys.append(rk_hex)
        
    return bits_to_string(ct_bits).hex(), str_round_keys
...
def decrypt(ciphertext_hex, key_str):
    ...
    key_bytes = key_bytes[:8]
    key_bits = string_to_bits(key_bytes)
    round_keys = generate_keys(key_bits)[::-1] # Reverse keys for decryption
    
    ct_bytes = bytes.fromhex(ciphertext_hex)
    ct_bits = string_to_bits(ct_bytes)
    
    pt_bits = []
    for i in range(0, len(ct_bits), 64):
        block = ct_bits[i:i+64]
        pt_bits.extend(des_block(block, round_keys))
        
    pt_bytes = list(bits_to_string(pt_bits))
    pad_len = pt_bytes[-1]
    pt_bytes = pt_bytes[:-pad_len]
    
    return bytes(pt_bytes).decode('utf-8', errors='ignore')
