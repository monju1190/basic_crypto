...
export default function DesView() {
  ...
  const generateKey = () => {
    // Generate an 8 byte random string
  ...
  return (
    <div className="h-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-2">
      <div className="flex-shrink-0">
        <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight font-heading">DES (Data Encryption Standard)</h2>
        <p className="text-gray-600 mt-1">Full 16-round DES block cipher implementation from scratch.</p>
      </div>
        ...
        <div className="bg-white/40 backdrop-blur-2xl p-6 rounded-2xl border border-white/60 shadow-[0_8px_32px_rgba(0,0,0,0.04)] overflow-hidden flex flex-col">
          ...
          {!result ? (
            ...
            <div className="space-y-2 overflow-y-auto flex-1 pr-2 custom-scrollbar">
              ...
              {result.round_keys && (
                <div className="mt-4">
                  <div className="text-xs text-gray-600 uppercase tracking-wider mb-3">16 Round Keys</div>
                  <div className="bg-white/60 p-3 rounded-xl border border-gray-200 text-xs font-mono text-gray-900 space-y-2">
                    {result.round_keys.map((rk: string, i: number) => (
                      <div key={i} className="flex border-b border-gray-200 pb-2">
                        ...
                        <span className="text-amber-600">{rk}</span>
                      </div>
                    ))}
                  </div>
                </div>
